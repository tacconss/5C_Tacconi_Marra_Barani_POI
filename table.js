    ...
    render: () => {
      ...
      htmlTable += data.map((row, index) => {
        if (index === 0) {
          ...
          // Riga con bottoni per le altre righe
      ...
      eliminaButtons.forEach((button) => {
        button.onclick = () => {
          ...
          data.splice(id + 1, 1); // Rimuove la riga dalla tabella (offset di 1 per l'intestazione)
          parentElement.innerHTML = ''; // Cancella la tabella prima di renderla di nuovo
          this.render(); // Re-renderizza la tabella aggiornata
        };
      });
    }
  }
